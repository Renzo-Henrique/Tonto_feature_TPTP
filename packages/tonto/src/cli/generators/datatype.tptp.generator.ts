...
export function customDataTypeTptpGenerator(dataType: DataType, model: Package): Class {
    const dataTypeClass = model.createDatatype(dataType.name);

    return dataTypeClass;
}

export function customDataTypeAttributesTptpGenerator(dataType: DataType, dataTypes: Class[]) {
    ...
    if (dataTypeClass) {
        dataType.attributes.forEach((element) => {
            ...
            if (createdAttribute) {
                ...
                setPropertyCardinalityTptp(element.cardinality, createdAttribute);
