...
import { generalizationTptpGenerator } from "./class.tptp.generator.js";
import { relationGeneralizationTptpGenerator } from "./relation.tptp.generator.js";

//TODO:: verificar corretude do arquivo

export function generateDataTypeSpecializationsTptp(
    ...
    declaratedDataTypes.forEach((declaration) => {
        ...
        dataType.specializationEndurants.forEach((endurant) => {
            ...
            if (targetDataType) {
                generalizationTptpGenerator(packageItem, sourceDataType, targetDataType);
            } else {
                console.log(
                    chalk.yellow(
...
export function generateTptpSpecializations(
    contextModule: ContextModule,
    classes: Class[],
    relations: Relation[],
    packageItem: Package
        ...
        if (declaration.$type === "ClassDeclaration") {
            ...
            if (classElement.specializationEndurants.length > 0) {
                ...
                if (sourceClass) {
                    classElement.specializationEndurants.forEach((endurant) => {
                        ...
                        if (targetClass) {
                            generalizationTptpGenerator(packageItem, targetClass, sourceClass);
                            generateInternalRelationSpecializationTptp(classElement, relations, packageItem);
                        } else {
                            console.log(
                                chalk.yellow(
            ...
            if (elementRelation.specializeRelation) {
                ...
                if (elementRelationCreated && targetRelation) {
                    relationGeneralizationTptpGenerator(packageItem, elementRelationCreated, targetRelation);
                } else {
                    console.log(
                        chalk.yellow(
...
export function generateInternalRelationSpecializationTptp(
